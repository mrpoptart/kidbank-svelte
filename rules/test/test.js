...
describe("Child Deleting", async () => {
    it(`authed user can delete their child`, async () => {
        let adminDb = getAdminDb();
        await adminDb.collection('children').doc('23456').set({
            name: "Bob",
            parents: ['success@kidbank.com']
        ...
        const db = await getAuthenticatedDb();
        await assertSucceeds(db.collection('children').doc('23456').delete());
    })
    it(`authed user can't delete a child that's not theirs`, async () => {
        let adminDb = getAdminDb();
        await adminDb.collection('children').doc('23456').set({
            name: "Bob",
            parents: ['fail@kidbank.com']
        ...
        const db = await getAuthenticatedDb();
        await assertFails(db.collection('children').doc('23456').delete());
    })
});
...
describe("Transactions", async () => {
    it(`authed user can add a transaction to their child`, async () => {
        let adminDb = getAdminDb();
        await adminDb.collection('children').doc('23456').set({
            name: "Bob",
            parents: ['success@kidbank.com'],
        ...
        const db = await getAuthenticatedDb();
        await setDoc(doc(db, 'children', '23456', 'transactions', '123456'), {
            "amount": 1,
            "date": "2023-01-24T14:00:39",
            "name": "fdsa",
            "spendFrom": "spend"
        })
    })
});
describe("Seeds Data", async () => {
    it(`seeds a kid`, async () => {
        let adminDb = getAdminDb();
        await adminDb.collection('children').doc('C_ZwnhLTqaVuGlahO5bTw').set({
            "birthday": "2014-11-13",
            "interest": 10,
            "name": "Malcolm",
            "parents": [
